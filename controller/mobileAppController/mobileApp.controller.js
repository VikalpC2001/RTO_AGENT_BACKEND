...
const getInsuranceExpiredBookList = async(req,res)=>{
    ...
    try{
        ...
        if(req.query.apiNumber === '1'){
            ...
            sql_insuranceExpired_getdetails = `SELECT vehicleRegistrationId ,vehicleRegistrationNumber, CONCAT(buyerFirstName,' ',buyerLastName) AS buyerName, clientWhatsAppNumber,DATE_FORMAT(insuranceStartDate, '%d %M %Y') AS insuranceStartDate, DATE_FORMAT(insuranceEndDate, '%d %M %Y') AS insuranceEndDate FROM vehicle_registration_details
